#!/bin/bash
#SBATCH --job-name=qwen25-resume-art-e
#SBATCH --partition=a3megatpa
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:2
#SBATCH --cpus-per-task=26
#SBATCH --output=logs/train_qwen25_resume_%j.log
#SBATCH --error=logs/train_qwen25_resume_%j.err
#SBATCH --exclude=megatpa-a3meganodeset-0

set -euo pipefail

echo "=========================================="
echo "ART-E Training Job (Qwen2.5-14B Resume)"
...
# Step 2: Run training with Qwen2.5-14B (resuming from checkpoint)
echo "Starting training (Qwen2.5-14B-Instruct resume)..."
uv run python art_e/train.py --model qwen25-14b
